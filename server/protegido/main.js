...
function ativarCarrosselProdutos() {
  document.querySelectorAll('.produto.carrossel').forEach(produto => {
    ...
    if (thumbsContainer) {
      ...
      variacoes.forEach((variacao, index) => {
        ...
        thumb.src = variacao.img || variacao.imagem;
    ...
    const updateProduto = () => {
      imgGrande.src = variacoes[current].img || variacoes[current].imagem;
      nomeProduto.textContent = variacoes[current].nome;
      precoProduto.textContent = `R$ ${parseFloat(variacoes[current].preco).toFixed(2)}`;
